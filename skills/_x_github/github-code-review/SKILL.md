...
Perform code reviews on local changes before pushing or reviewing open PRs on GitHub. Most of this skill uses plain `git` — the `gh`/`curl` split only matters for PR-level interactions.

## Prerequisites

- Authenticated with GitHub/gh CLI
...
**ALWAYS** check what's the main/default branch - do NOT just assume `main` or `master`. Everything below uses `main` just as reference.
...
### Step 3: Read the diff and understand changes
...
### Step 4: Run automated checks locally (if applicable)
...
### Step 7: Also post a summary comment
