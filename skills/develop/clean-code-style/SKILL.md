...
description: Apply clean code style preferences when writing or refactoring code, including phase-oriented structure, semantic whitespace, explicit control flow, early returns, cohesive operations, deliberate syntax. When you are in review mode, remain in read-only mode - do not apply but report the problems.
...
- always follow the codebase's style and style preferences like:
  + use double quotes if the codebase uses double quotes.
  + use single quotes if it uses single quotes.
- never modify unrelated code to what your current exact task is.

## Organize by purpose

- Give each function one clear responsibility. But do not extract into one-line functions that does nothing but return a string, or similar.
- Use shared `src/utils.ts` files (or similar place, it may be a shared monorepo package) for reusable non-scoped utility-like functions.
  + Primary examples: bytesToHex, base64encode / base64url.
  + If the project needs similar, it most likely already has an installed dependency that has them - usual case: project uses viem/noble libraries, then just import from there and don't write your own helpers.
...
Here, the first two guards are one provider-validation phase. The client lookup begins a new phase, so it is separated.
...
Here, everything is clamped, and does not have curly braces for control flow blocks. Never skip curly braces, UNLESS there are too many `if`s with single-line body which leads to noise of too simple code with too many braces.

```ts
const provider = parseProvider(value);
if (!provider) return oauthError("invalid_request", "unsupported provider");
if (!enabledProviders(env).includes(provider)) return oauthError("invalid_request", "provider unavailable");
...
- Never skip curly braces, UNLESS there are too many `if`s with single-line body which leads to noise of too simple code with too many braces.
